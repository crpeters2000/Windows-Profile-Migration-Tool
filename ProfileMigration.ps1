...
# =============================================================================
# USER PROFILE TRANSFER TOOL
# Tested 100% working on Windows 11 25H2 (26200.7623)
# =============================================================================
Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
...
$global:DebugCheckBox = $null  # Debug mode checkbox for verbose 7-Zip logging

# === THEME SYSTEM ===
$global:CurrentTheme = $null  # Will be set to 'Light' or 'Dark'

# Theme color definitions
$Themes = @{
    Light = @{
        FormBackColor                 = [System.Drawing.Color]::FromArgb(245, 245, 245)
        PanelBackColor                = [System.Drawing.Color]::White
        HeaderBackColor               = [System.Drawing.Color]::White
        HeaderTextColor               = [System.Drawing.Color]::FromArgb(0, 120, 212)
        SubHeaderTextColor            = [System.Drawing.Color]::FromArgb(100, 100, 100)
        ButtonPrimaryBackColor        = [System.Drawing.Color]::FromArgb(0, 120, 212)
        ButtonPrimaryForeColor        = [System.Drawing.Color]::White
        ButtonPrimaryHoverBackColor   = [System.Drawing.Color]::FromArgb(16, 110, 190)
        ButtonSecondaryBackColor      = [System.Drawing.Color]::FromArgb(80, 80, 80)
        ButtonSecondaryForeColor      = [System.Drawing.Color]::White
        ButtonSecondaryHoverBackColor = [System.Drawing.Color]::FromArgb(60, 60, 60)
        ButtonSuccessBackColor        = [System.Drawing.Color]::FromArgb(16, 124, 16)
        ButtonSuccessForeColor        = [System.Drawing.Color]::White
        ButtonSuccessHoverBackColor   = [System.Drawing.Color]::FromArgb(12, 100, 12)
        ButtonDangerBackColor         = [System.Drawing.Color]::FromArgb(232, 17, 35)
        ButtonDangerForeColor         = [System.Drawing.Color]::White
        ButtonDangerHoverBackColor    = [System.Drawing.Color]::FromArgb(200, 15, 30)
        TextBoxBackColor              = [System.Drawing.Color]::White
        TextBoxForeColor              = [System.Drawing.Color]::Black
        LabelTextColor                = [System.Drawing.Color]::FromArgb(30, 30, 30)
        LogBoxBackColor               = [System.Drawing.Color]::FromArgb(250, 250, 250)
        LogBoxForeColor               = [System.Drawing.Color]::Black
        ProgressBarForeColor          = [System.Drawing.Color]::FromArgb(0, 120, 212)
        BorderColor                   = [System.Drawing.Color]::FromArgb(200, 200, 200)
    }
    Dark  = @{
        FormBackColor                 = [System.Drawing.Color]::FromArgb(32, 32, 32)
        PanelBackColor                = [System.Drawing.Color]::FromArgb(45, 45, 45)
        HeaderBackColor               = [System.Drawing.Color]::FromArgb(45, 45, 45)
        HeaderTextColor               = [System.Drawing.Color]::FromArgb(0, 122, 204)
        SubHeaderTextColor            = [System.Drawing.Color]::FromArgb(150, 150, 150)
        ButtonPrimaryBackColor        = [System.Drawing.Color]::FromArgb(0, 122, 204)
        ButtonPrimaryForeColor        = [System.Drawing.Color]::White
        ButtonPrimaryHoverBackColor   = [System.Drawing.Color]::FromArgb(16, 110, 190)
        ButtonSecondaryBackColor      = [System.Drawing.Color]::FromArgb(80, 80, 80)
        ButtonSecondaryForeColor      = [System.Drawing.Color]::White
        ButtonSecondaryHoverBackColor = [System.Drawing.Color]::FromArgb(100, 100, 100)
        ButtonSuccessBackColor        = [System.Drawing.Color]::FromArgb(16, 124, 16)
        ButtonSuccessForeColor        = [System.Drawing.Color]::White
        ButtonSuccessHoverBackColor   = [System.Drawing.Color]::FromArgb(12, 100, 12)
        ButtonDangerBackColor         = [System.Drawing.Color]::FromArgb(232, 17, 35)
        ...
        LabelTextColor                = [System.Drawing.Color]::FromArgb(200, 200, 200)
        LogBoxBackColor               = [System.Drawing.Color]::FromArgb(20, 20, 20)
        LogBoxForeColor               = [System.Drawing.Color]::White
        ProgressBarForeColor          = [System.Drawing.Color]::FromArgb(0, 122, 204)
...
# Function to detect profile type (Local, Domain, or AzureAD)
# Function to convert Entra/AzureAD ObjectId to Windows SID
...
# Function to get AzureAD user SID using Microsoft Graph
...
# === CPU CORE DETECTION ===
# Detect number of CPU cores for optimal multi-threading (Win10 compatibility)
try {
    $cpuCores = (Get-CimInstance Win32_ComputerSystem -ErrorAction Stop).NumberOfLogicalProcessors
}
catch {
    try {
        $cpuCores = (Get-WmiObject Win32_ComputerSystem -ErrorAction Stop).NumberOfLogicalProcessors
    }
    catch {
        $cpuCores = 4  # Fallback to 4 if detection fails
    }
}
...
$Config = @{
    Version                     = "v2.12.25" # Refactoring Phase 4 (Cleanup Wizard)
...
if (-not $sevenZipPath) {
    ...
    # Check winget health first
    $health = Test-WingetFunctionality
    if (-not $health.Success) {
        Write-Host "Winget issue detected: $($health.Error). Attempting repair..." -ForegroundColor Yellow
        Repair-WingetSources | Out-Null
    }

    try {
        Write-Host "  - Executing winget install for 7-Zip..." -ForegroundColor Gray
        $proc = Start-Process "winget.exe" -ArgumentList "install", "--id", "7zip.7zip", "--source", "winget", "--silent", "--accept-package-agreements", "--accept-source-agreements", "--force" -Wait -PassThru -WindowStyle Hidden
        
        # Wait for up to 60 seconds for installation
        $timeout = 60
        $timer = [System.Diagnostics.Stopwatch]::StartNew()
        while (-not $proc.HasExited -and $timer.Elapsed.TotalSeconds -lt $timeout) {
            Start-Sleep -Milliseconds 500
        ...
        if (-not $proc.HasExited) {
            $proc.Kill()
            Write-Host "  - Installation timed out. Please install 7-Zip manually." -ForegroundColor Red
        }
        elseif ($proc.ExitCode -eq 0 -or $proc.ExitCode -eq -1978335189) {
...
while (-not $sevenZipPath) {
    $message = "7-Zip is required for compressing and extracting profile archives.`r`n`r`n" +
    ...
    switch ($result.Action) {
        ...
        "Retry" {
            ...
            # Full troubleshooting before retry
            $health = Test-WingetFunctionality
            if (-not $health.Success) {
                Log-Warning "Winget check failed: $($health.Error). Running repair..."
                Repair-WingetSources | Out-Null
            }
            ...
            try {
                ...
                if ($proc.ExitCode -eq 0 -or $proc.ExitCode -eq -1978335189) {
                    ...
                    if ($sevenZipPath) {
                        ...
                        Show-ModernDialog -Message "7-Zip is now ready!" -Title "Success" -Type Success -Buttons OK
                    }
                    else {
                        Show-ModernDialog -Message "Installation completed but 7z.exe not found in expected location.`r`n`r`nPlease use 'Browse for 7z.exe' option." -Title "Manual Location Required" -Type Warning -Buttons OK
                    }
                }
                else {
                    Show-ModernDialog -Message "Winget installation failed (exit code: $($proc.ExitCode)).`r`n`r`nTry downloading from website or browse for existing installation." -Title "Installation Failed" -Type Error -Buttons OK
                }
            }
            catch {
...
"@
        ...
        $html += @"
        <div class="footer">
            <p>Profile Migration Tool 2.12.20 | Generated on $(hostname) | PowerShell $($PSVersionTable.PSVersion)</p>
        </div>
    </div>
</body>
</html>
...
foreach ($j in $junctions) {
    # 7-Zip -x! patterns are relative to the added root; we are adding "$($userProfile.Path)\*"
...
try {
    ...
    if ($rpItems.Count -gt 0) {
        ...
        foreach ($p in $preview) {
            $relPreview = $p.FullName.Substring($userProfile.Path.Length).TrimStart('\\')
...
$7zArgs = @('a', '-tzip', $ZipPath, "$($userProfile.Path)\*", '-mx=5', '-mmt=on', '-bsp1') + $7zExclusions
...
# =============================================================================
# PROFILE TYPE CONVERSION FUNCTIONS
# =============================================================================

# Get the type of a user profile (Local, Domain, or AzureAD)
...
# Test all preconditions for profile conversion
...
# Add Active Setup registry key to re-register AppX packages on first login
# This runs BEFORE the desktop loads, fixing file locking issues with Search/Start Menu
...
# Update ProfileList registry entry for profile conversion
...
# Repair an existing user profile (permissions and registry)
...
# Helper to get uncompressed size of a ZIP file using 7-Zip listing
...
# =============================================================================
# CREDENTIAL HELPER WITH RETRY LOGIC
# =============================================================================
...
$global:Form.Text = "Profile Migration Tool v$($Config.Version)"
...
Log-Message "Profile Transfer Tool v$($Config.Version)"
